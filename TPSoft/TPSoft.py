...
class Partícula:
    ...
    def plotar(self, TPC, ax):
        ...
        t = np.arange(0, encontrar_limite(self.teta, self.fi, self.x_emissão, self.y_emissão, self.z_emissão, TPC))
...
class TPC:
    ...
    def coleção_de_luz(self, Particula, delta, ax, **kwargs):
        """Essa função plota a coleção de fótons em uma parede da TPC na forma de um histograma ou scatter."""
        ...
        while 0 <= x <= self.comprimento_x and 0 <= y <= self.comprimento_y and 0 <= z <= self.comprimento_z:
            ...
            intersecção_plano_x, intersecção_plano_y, intersecção_plano_z = intersecção_foton_plano(intersecção_limites, x_plano, y_plano, z_plano)
...
def intersecção_foton_plano(intersecção_limites, x_plano, y_plano, z_plano):
    ...
    for i in range(len(intersecção_limites)):
        ...
        if x_plano != None and round(intersecção_limites[i][0]) == x_plano:
        ...
        elif y_plano != None and round(intersecção_limites[i][1]) == y_plano:
    
            intersecção_plano_x += [intersecção_limites[i][0]]
            intersecção_plano_y += [intersecção_limites[i][1]]
            intersecção_plano_z += [intersecção_limites[i][2]]
    
        elif z_plano != None and round(intersecção_limites[i][2]) == z_plano:
